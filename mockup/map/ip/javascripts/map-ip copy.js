...
var detailBaseURL = "detail.html?";	// base URL for the normal map detail screen
var fullscreenBaseURL = "detail-fullscreen.html?";	// base URL for the fullscreen map detail screen
var mapLayers = "Towns,Hydro,Greenspace,Sport,Courtyards,Roads,Rail,Landmarks,Parking,Other+Buildings,Buildings,bldg-iden-14,road-iden-14,greens-iden-12,landmarks-iden-12";
var mapOptions = "&selectfield=facility&selectlayer=Buildings";
...
// Loads an image from the given URL into the image with the specified ID
	...
	if(objFullscreen) {
		objFullscreen.href = getMapURL(fullscreenBaseURL);
	}
	var objSmallscreen = document.getElementById("smallscreen");
	if(objSmallscreen) {
		objSmallscreen.href = getMapURL(detailBaseURL);
	}
}


function getMapURL(strBaseURL) {
...
function recenter() {
// Reset the map image to its initial coordinates -- only if it's not already there
	if(hasMoved) {
		hasMoved = false;
		mapBoxW = initMapBoxW;
		mapBoxN = initMapBoxN;
...
// Check to see if the map has been moved (zoomed or scrolled) away from its initial position, and disable/enable the 'recenter' button accordingly
	hasMoved = !((mapBoxW == initMapBoxW) && (mapBoxN == initMapBoxN) && (mapBoxS == initMapBoxS) && (mapBoxE == initMapBoxE));
	if(hasMoved) {
		enable('recenter');
	} else {
		disable('recenter');
...
// Applies full-screen map-option changes and hides the form
	var newLayers = "Towns,Hydro";
	
	// Code to manipulate the string newLayers should go here, based on what the user toggled in the form
	
	if(document.mapform.chkCourts.checked) { newLayers = newLayers + "," + document.mapform.chkCourts.value; }
	if(document.mapform.chkRoads.checked) { newLayers = newLayers + "," + document.mapform.chkRoads.value; }
	if(document.mapform.chkRail.checked) { newLayers = newLayers + "," + document.mapform.chkRail.value; }
	if(document.mapform.chkLandmarks.checked) { newLayers = newLayers + "," + document.mapform.chkLandmarks.value; }
	if(document.mapform.chkParking.checked) { newLayers = newLayers + "," + document.mapform.chkParking.value; }
	if(document.mapform.chkBuildings.checked) { newLayers = newLayers + "," + document.mapform.chkBuildings.value; }
