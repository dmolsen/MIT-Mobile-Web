...
if (preg_match("/^".$regex."/i",$_SERVER["HTTP_REFERER"])) {
	$expire=time()+60*60; # cookie is set for 60 minutes
	setcookie("mobi_referral", "true", $expire);
}
else if (isset($_COOKIE["mobi_referral"])) { 
	# don"t do anything, just pass on
}
else if ($_REQUEST["nomobi"] == "true") {
	# this offers the ability to set links up so they don"t require cookie or referral check
	# might want to have this set the cookie too... not sure
	# don"t do anything, just pass on
}
else {
